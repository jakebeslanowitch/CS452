/**
 * Implementation of a memory allocator based on the Malloc System.
...
int buddy_init(size_t size) { 
	...
	if(size<=0) {
		...
		pool.size = (1<<Log2l(size));
	}

	pool.lgsize=Log2l(pool.size);
    pool.start=sbrk(pool.size);
	if(pool.start==(void *)-1) {
		return -1;
	}
	// init each pool freeList to values
	...
	// delare struct pointers from pool.start to members of block_header
...
void *malloc(size_t size) {
	...
	// sets the block tag back to RESERVED
	block->tag=RESERVED;
