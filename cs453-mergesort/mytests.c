...
int main(int argc, char **argv) {
	...
	if (argc < 3) { // there must be at least one command-line argument
		fprintf(stderr, "Usage: %s <input size> <number of levels of thread creation> <[<seed>] \n", argv[0]);
		exit(1);
	}
	...
	int *B = (int *) malloc(sizeof(int) * (n+1)); // n+1 since we are using B[1]..B[n]
	...
	// sort the input (and time it)
	start_time = getMilliSeconds();
	parallel_mergesort(B,1,n, numThreads);
	sorting_time = getMilliSeconds() - start_time;
